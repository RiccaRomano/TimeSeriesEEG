...
% The following code simulates the in-sample AR model fit against the candidate signal
...
load Results_AR.mat % Load optimal AR model of the candidate signal
Fs=256; % Frames per second
tspan=(EstMdl.P+1:length(dataTrain))/Fs; % Time span of the in-sample period
residual=infer(EstMdl,dataTrain(EstMdl.P+1:end),'Y0',dataTrain(1:EstMdl.P)); % Estimate the in-sample residuals
...
% Simulate individual AR model forecasts
numobs=length(dataTest);
numpaths=1000;
[ysim,~]=simulate(EstMdl,numobs,'Y0',dataTrain,'NumPaths',numpaths);
...
save('Forecast_AR.mat','power_err','hist_err','pear_corr');

saveas(f1,'InSampleFit_AR.fig');
saveas(f1,'InSampleFit_AR.svg');
saveas(f1,'InSampleFit_AR.eps');

saveas(f2,'PowerSpec_AR.fig');
saveas(f2,'PowerSpec_AR.svg');
saveas(f2,'PowerSpec_AR.eps');

saveas(f3,'Hist_AR.fig');
saveas(f3,'Hist_AR.svg');
saveas(f3,'Hist_AR.eps');

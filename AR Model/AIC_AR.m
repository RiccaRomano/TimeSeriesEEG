...
% The following code performs an AIC search on an AR model of the candidate signal (participant 9, trial 5 , interval 1)

clear
close all
clc

load CandidateSignal.mat dataTrain % Load the training period of the candidate signal 
    ...
    while exitflag==0                                                                                                                                                   
        if optionset==1
            try
                ...
                [EstMdl,EstParamCov,logL,info]=estimate(Mdl,dataTrain(Mdl.P+1:end),'Y0',dataTrain(1:Mdl.P),'Display','full','Options',options);

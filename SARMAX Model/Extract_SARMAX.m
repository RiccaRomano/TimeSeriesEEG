...
% The following code holds information on extracting the optimal SARMAX model for the candidate signal (participant 9, trial 5, interval 1)
% Optimal model orders are informed by AIC (from the AIC_SARMAX.m script)
...
load AIC_SARMAX.mat % Load AIC search results for the candidate signal ARMAX model
load CandidateSignal.mat dataTrain dataTest stimTrain stimTest % Load training data of the candidate signal

% Optimal SARMAX model order extracted from the AIC matrix, first entry = 1 AR term
[AR_Order,MA_Order]=find(AICmat==min(min(AICmat)));
AR_Order=AR_Order-1;
MA_Order=MA_Order-1;
temp_ARLags=AR_Lags(1:AR_Order); % AR terms only regress against signficant lags
temp_MALags=MA_Lags(1:MA_Order); % MA terms only regress against significant lags
...
try
    % Keep track of optimization algorithm iterations
    hist.fval=[];
    hist.coef=[];
    hist.feas=[];
    hist.step=[];
    hist.opt=[];
    searchdir=[];
    [EstMdl,EstParamCov,logL,info,Extra]=estimate_mod_hist(Mdl,dataTrain(Mdl.P+1:end),'Y0',dataTrain(1:Mdl.P),'X',stimTrain,'Display','iter','Options',options);
    summarize_mod_hist(EstMdl,Extra);
    history=hist;
...
save('Results_SARMAX.mat','dataTrain','dataTest','stimTrain','stimTest','EstMdl','EstParamCov','logL','info','coefs_roots');

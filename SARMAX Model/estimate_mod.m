...
%
     ...
     if (numel(LagsAR) + numel(LagsSAR) + numel(LagsMA) + numel(LagsSMA)) == 0
         ...
         %conStruct.nonLinear = @(x) arimaNonLinearConstraints_mod(x, LagsAR, LagsSAR, LagsMA, LagsSMA, tolerance);
     end
...
%
        ...
        f = @(x) [arimaNonLinearConstraints(x(1:i), LagsAR, LagsSAR, LagsMA, LagsSMA, tolerance) ; 
...
%
  % 
  % try
  %   Mdl.Constant=coefficients(1);
  %   Mdl.Beta=coefficients(iBeta);
  %   Mdl.Variance=coefficients(end);
  %   Mdl.SAR{LagsSAR}=coefficients(iSAR);
  % 
  %   if isempty(LagsAR) && ~isempty(LagsMA) %Just MA
  %       for ii=1:length(LagsMA)
  %           Mdl.MA{LagsMA(ii)}=coefficients(iMA(ii));
  %       end
  %   elseif isempty(LagsMA) && ~isempty(LagsAR) % Just AR
  %       for ii=1:length(LagsAR)
  %           Mdl.AR{LagsAR(ii)}=coefficients(iAR(ii));
  %       end
  %   else % ARMA
  %       for ii=1:length(LagsMA)
  %           Mdl.MA{LagsMA(ii)}=coefficients(iMA(ii));
  ...
  %   Extra.FitInformation.StandardErrors.AR=cell(1,LagsAR(end));
  %   Extra.FitInformation.StandardErrors.MA=cell(1,LagsMA(end));
  %   Extra.FitInformation.StandardErrors.SAR=cell(1,LagsSAR(end));
  %   Extra.FitInformation.StandardErrors.SAR{LagsSAR}=SE(iSAR);
  % 
  %   if isempty(LagsAR) && ~isempty(LagsMA) % Just MA
  %       for ii=1:length(LagsMA)
  %           Extra.FitInformation.StandardErrors.MA{LagsMA(ii)}=SE(iMA(ii));
  %       end
  %   elseif ~isempty(LagsAR) && isempty(LagsMA)
  %       for ii=1:length(LagsAR)
  %           Extra.FitInformation.StandardErrors.AR{LagsAR(ii)}=SE(iAR(ii));
  %       end
  %   else
  %        for ii=1:length(LagsMA)
  %           Extra.FitInformation.StandardErrors.MA{LagsMA(ii)}=SE(iMA(ii));
  ...
  %   Extra.LagOpLHS{1,1}.Degree=LagsAR(end);
  %   Extra.LagOpLHS{1,2}.Lags=[0 LagsSAR];
  %   Extra.LagOpLHS{1,2}.Degree=P;
  % 
  %   Extra.LagOpRHS{1,1}.Lags=[0 LagsMA];
  %   Extra.LagOpRHS{1,1}.Degree=LagsMA(end);
  %   Extra.LagOpRHS{1,2}.Lags=[0 LagsSMA];
  %   Extra.LagOpRHS{1,2}.Degree=Q;
  % 
  % catch old

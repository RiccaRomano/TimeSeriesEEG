...
% The following code simulates the in-sample ARMAX model fit against the candidate signal
...
load Results_ARMAX.mat % Load optimal ARMAX model of the candidate signal
...
% Simulate individual ARMAX model forecasts
numobs=length(dataTest);
numpaths=1000;
[ysim,~]=simulate_mod(EstMdl,numobs,'Y0',dataTrain,'X0',stimTrain,'XF',stimTest,'NumPaths',numpaths);
...
save('Forecast_ARMAX.mat','power_err','hist_err','pear_corr');

saveas(f1,'InSampleFit_ARMAX.fig');
saveas(f1,'InSampleFit_ARMAX.svg');
saveas(f1,'InSampleFit_ARMAX.eps');

saveas(f2,'PowerSpec_ARMAX.fig');
saveas(f2,'PowerSpec_ARMAX.svg');
saveas(f2,'PowerSpec_ARMAX.eps');

saveas(f3,'Hist_ARMAX.fig');
saveas(f3,'Hist_ARMAX.svg');
saveas(f3,'Hist_ARMAX.eps');
